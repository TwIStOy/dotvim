_: let
  ...
  picker = method: "require('dotvim.features.snacks_picker').${method}()";
...
in {
  ...
  keymaps = [
    ...
    # Snacks picker
    (nraw "<leader>e" (picker "find_files") "edit-project-files")
    (nraw "g/" (picker "live_grep") "live-grep")
    (nraw "<leader>lg" (picker "live_grep") "live-grep")
    (nraw "<F4>" (picker "buffers") "all-buffers")
