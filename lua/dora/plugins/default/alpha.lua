...
local function make_button(sc, txt, callback, opts)
  opts = opts or {}
  local on_press =
    lib.fn.normalize_callback(callback, vim.F.if_nil(opts.feedkeys, true))
...
local function make_recent_files_buttons()
  ...
  local function mru(start, cwd, items_number, opts)
    ...
    for i, fn in ipairs(oldfiles) do
      ...
      local file_button_el = make_file_button(fn, " " .. shortcut, short_fn)
...
local function make_recent_project_buttons()
  local function _build_buttons()
    local recent_projects = lib.fn.require_then(
      "project_nvim",
    ...
    for i, project_path in ipairs(recent_projects) do
      ...
      buttons[#buttons + 1] = make_project_button(" " .. shortcut, project_path)
    end
...
local function make_header_liens()
  local lines = {}
  for i, line in ipairs(config.hello.header_text) do
    lines[i] = {
      type = "text",
      val = line,
      opts = {
