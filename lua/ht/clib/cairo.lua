...
map("CAIRO_STATUS_",
    {"SUCCESS", "NO_MEMORY", "INVALID_RESTORE", "INVALID_POP_GROUP", "NO_CURRENT_POINT", "INVALID_MATRIX",
     "INVALID_STATUS", "NULL_POINTER", "INVALID_STRING", "INVALID_PATH_DATA", "READ_ERROR", "WRITE_ERROR",
     "SURFACE_FINISHED", "SURFACE_TYPE_MISMATCH", "PATTERN_TYPE_MISMATCH", "INVALID_CONTENT", "INVALID_FORMAT",
     "INVALID_VISUAL", "FILE_NOT_FOUND", "INVALID_DASH", "INVALID_DSC_COMMENT", "INVALID_INDEX",
     "CLIP_NOT_REPRESENTABLE", "TEMP_FILE_ERROR", "INVALID_STRIDE", "FONT_TYPE_MISMATCH", "USER_FONT_IMMUTABLE",
     "USER_FONT_ERROR", "NEGATIVE_COUNT", "INVALID_CLUSTERS", "INVALID_SLANT", "INVALID_WEIGHT", "INVALID_SIZE",
     "USER_FONT_NOT_IMPLEMENTED", "DEVICE_TYPE_MISMATCH", "DEVICE_ERROR", "INVALID_MESH_CONSTRUCTION",
     "DEVICE_FINISHED", "LAST_STATUS"})

map("CAIRO_CONTENT_", {"COLOR", "ALPHA", "COLOR_ALPHA"})

map("CAIRO_FORMAT_", {"INVALID", "ARGB32", "RGB24", "A8", "A1", "RGB16_565", "RGB30"})
...
map("CAIRO_OPERATOR_",
    {"CLEAR", "SOURCE", "OVER", "IN", "OUT", "ATOP", "DEST", "DEST_OVER", "DEST_IN", "DEST_OUT", "DEST_ATOP", "XOR",
     "ADD", "SATURATE", "MULTIPLY", "SCREEN", "OVERLAY", "DARKEN", "LIGHTEN", "COLOR_DODGE", "COLOR_BURN", "HARD_LIGHT",
     "SOFT_LIGHT", "DIFFERENCE", "EXCLUSION", "HSL_HUE", "HSL_SATURATION", "HSL_COLOR", "HSL_LUMINOSITY"})
...
map("CAIRO_ANTIALIAS_", {"DEFAULT", "NONE", "GRAY", "SUBPIXEL", "FAST", "GOOD", "BEST"})

cr.antialias = getset_func(C.cairo_get_antialias, C.cairo_set_antialias, "CAIRO_ANTIALIAS_")

map("CAIRO_FILL_RULE_", {"WINDING", "EVEN_ODD"})
...
map("CAIRO_TEXT_CLUSTER_FLAG_", {"BACKWARD"})

map("CAIRO_FONT_SLANT_", {"NORMAL", "ITALIC", "OBLIQUE"})

map("CAIRO_FONT_WEIGHT_", {"NORMAL", "BOLD"})

map("CAIRO_SUBPIXEL_ORDER_", {"DEFAULT", "RGB", "BGR", "VRGB", "VBGR"})

map("CAIRO_HINT_STYLE_", {"DEFAULT", "NONE", "SLIGHT", "MEDIUM", "FULL"})

map("CAIRO_HINT_METRICS_", {"DEFAULT", "OFF", "ON"})
...
fopt.subpixel_order = getset_func(C.cairo_font_options_get_subpixel_order, C.cairo_font_options_set_subpixel_order,
    "CAIRO_SUBPIXEL_ORDER_")
fopt.hint_style = getset_func(C.cairo_font_options_get_hint_style, C.cairo_font_options_set_hint_style,
    "CAIRO_HINT_STYLE_")
fopt.hint_metrics = getset_func(C.cairo_font_options_get_hint_metrics, C.cairo_font_options_set_hint_metrics,
    "CAIRO_HINT_METRICS_")
...
cr.font_face = getset_func(C.cairo_get_font_face, function(cr, family, slant, weight)
    if type(family) == "string" then
        C.cairo_select_font_face(cr, family, X("CAIRO_FONT_SLANT_", slant or "normal"),
...
map("CAIRO_FONT_TYPE_", {"TOY", "FT", "WIN32", "QUARTZ", "USER"})
...
function sfont.text_to_glyphs(sfont, x, y, s, slen, glyphs, num_glyphs, clusters, num_clusters)
    ...
    local status = C.cairo_scaled_font_text_to_glyphs(sfont, x, y, s, slen or #s, glyphs_buf, num_glyphs_buf,
    ...
    if status == 0 then
        ...
        return glyphs, num_glyphs_buf[0], clusters, num_clusters_buf[0],
...
function M.toy_font_face(family, slant, weight)
    return ffi.gc(C.cairo_toy_font_face_create(family, X("CAIRO_FONT_SLANT_", slant), X("CAIRO_FONT_WEIGHT_", weight)),
...
face.init_func = getset_func(C.cairo_user_font_face_get_init_func, C.cairo_user_font_face_set_init_func)

face.render_glyph_func = getset_func(C.cairo_user_font_face_get_render_glyph_func,
    C.cairo_user_font_face_set_render_glyph_func)

face.text_to_glyphs_func = getset_func(C.cairo_user_font_face_get_text_to_glyphs_func,
    C.cairo_user_font_face_set_text_to_glyphs_func)

face.unicode_to_glyph_func = getset_func(C.cairo_user_font_face_get_unicode_to_glyph_func,
...
map("CAIRO_PATH_", {"MOVE_TO", "LINE_TO", "CURVE_TO", "CLOSE_PATH"})
...
function path.dump(p)
    print(string.format("cairo_path_t (length = %d, status = %s)", p.num_data, M.status_message(p.status)))
    local i = 0
...
map("CAIRO_DEVICE_TYPE_", {"DRM", "GL", "SCRIPT", "XCB", "XLIB", "XML", "COGL", "WIN32", "INVALID"})
...
map("CAIRO_SURFACE_OBSERVER_", {"NORMAL", "RECORD_OPERATIONS"})
...
map("CAIRO_SURFACE_TYPE_",
    {"IMAGE", "PDF", "PS", "XLIB", "XCB", "GLITZ", "QUARTZ", "WIN32", "BEOS", "DIRECTFB", "SVG", "OS2",
     "WIN32_PRINTING", "QUARTZ_IMAGE", "SCRIPT", "QT", "RECORDING", "VG", "GL", "DRM", "TEE", "XML", "SKIA",
     "SUBSURFACE", "COGL"})
...
sr.fallback_resolution = getset_func(d2out_func(C.cairo_surface_get_fallback_resolution),
...
function M.recording_surface(content, x, y, w, h)
    ...
    return ffi.gc(C.cairo_recording_surface_create(X("CAIRO_CONTENT_", content), x and r or nil),
...
patt.callback_data = getset_func(C.cairo_raster_source_pattern_get_callback_data,
...
patt.snapshot_function = getset_func(C.cairo_raster_source_pattern_get_snapshot,
    C.cairo_raster_source_pattern_set_snapshot)

patt.copy_function = getset_func(C.cairo_raster_source_pattern_get_copy, C.cairo_raster_source_pattern_set_copy)

patt.finish_function = getset_func(C.cairo_raster_source_pattern_get_finish, C.cairo_raster_source_pattern_set_finish)
...
map("CAIRO_PATTERN_TYPE_", {"SOLID", "SURFACE", "LINEAR", "RADIAL", "MESH", "RASTER_SOURCE"})
...
map("CAIRO_EXTEND_", {"NONE", "REPEAT", "REFLECT", "PAD"})

patt.extend = getset_func(C.cairo_pattern_get_extend, C.cairo_pattern_set_extend, "CAIRO_EXTEND_")

map("CAIRO_FILTER_", {"FAST", "GOOD", "BEST", "NEAREST", "BILINEAR", "GAUSSIAN"})
...
map("CAIRO_REGION_OVERLAP_", {"IN", "OUT", "PART"})
...
map("CAIRO_LCD_FILTER_", {"DEFAULT", "NONE", "INTRA_PIXEL", "FIR3", "FIR5"})

map("CAIRO_ROUND_GLYPH_POS_", {"DEFAULT", "ON", "OFF"})

fopt.lcd_filter = getset_func(_C._cairo_font_options_get_lcd_filter, _C._cairo_font_options_set_lcd_filter,
    "CAIRO_LCD_FILTER_")

fopt.round_glyph_positions = getset_func(_C._cairo_font_options_get_round_glyph_positions,
    _C._cairo_font_options_set_round_glyph_positions, "CAIRO_ROUND_GLYPH_POS_")
...
function cr:quad_curve_to(x1, y1, x2, y2)
    ...
    self:curve_to((x0 + 2 * x1) / 3, (y0 + 2 * y1) / 3, (x2 + 2 * x1) / 3, (y2 + 2 * y1) / 3, x2, y2)
end
...
function mt.tostring(mt)
    return string.format("[%12f%12f]\n[%12f%12f]\n[%12f%12f]", mt.xx, mt.yx, mt.xy, mt.yy, mt.x0, mt.y0)
end

function mt.equal(m1, m2)
    return type(m2) == "cdata" and m1.xx == m2.xx and m1.yy == m2.yy and m1.xy == m2.xy and m1.yx == m2.yx and m1.x0 ==
               m2.x0 and m1.y0 == m2.y0
end
...
function M.ft_font_face(ft_face, load_flags)
    ...
    local face = ffi.gc(C.cairo_ft_font_face_create_for_ft_face(ft_face, load_flags or 0), C.cairo_font_face_destroy)
    local status = C.cairo_font_face_set_user_data(face, key, ft_face, ffi.cast("cairo_destroy_func_t", function()
        ft_face:free()
...
ffi.metatype("cairo_t", {
    __index = cr
})
ffi.metatype("cairo_rectangle_list_t", {
    __index = rl
})
ffi.metatype("cairo_glyph_t", {
    __index = glyph
})
ffi.metatype("cairo_text_cluster_t", {
    __index = cluster
})
ffi.metatype("cairo_font_options_t", {
    __index = fopt
})
ffi.metatype("cairo_font_face_t", {
    __index = face
})
ffi.metatype("cairo_scaled_font_t", {
    __index = sfont
})
ffi.metatype("cairo_path_t", {
    __index = path
})
ffi.metatype("cairo_device_t", {
    __index = dev
})
ffi.metatype("cairo_surface_t", {
    __index = sr
})
ffi.metatype("cairo_pattern_t", {
    __index = patt
})
...
ffi.metatype("cairo_region_t", {
    __index = rgn
})

ffi.metatype("cairo_text_extents_t", {
    __tostring = function(t)
        return string.format("bearing: (%d, %d), height: %d, advance: (%d, %d)", t.x_bearing, t.y_bearing, t.width,
            t.height, t.x_advance, t.y_advance)
    end
})

ffi.metatype("cairo_font_extents_t", {
    __tostring = function(t)
        return string.format("ascent: %d, descent: %d, height: %d, max_advance: (%d, %d)", t.ascent, t.descent,
            t.height, t.max_x_advance, t.max_y_advance)
    end
})
