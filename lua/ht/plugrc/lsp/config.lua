...
M.config = function() -- code to run after plugin loaded
    ...
    menu:append_section("*", {Menu.item("LSP", {
        items = {Menu.item("Goto Declaration", {
            action = function()
    ...
    menu:append_section("cpp", {Menu.item("Symbol Info", {
        action = function()
    ...
    menu:append_section("rust", {Menu.item("Rust-tools", {
        items = {Menu.item("Hover Actions", {
            action = function()
  ...
  --]]
    ...
    require"clangd_extensions".setup {
        server = {
            cmd = {vim.g.compiled_llvm_clang_directory .. '/bin/clangd', '--clang-tidy', '--background-index',
                   '--background-index-priority=normal', '--ranking-model=decision_forest',
                   '--completion-style=detailed', '--header-insertion=iwyu', '--limit-references=100',
                   '--limit-results=100', '--include-cleaner-stdlib', '--malloc-trim', '-j=20'},
