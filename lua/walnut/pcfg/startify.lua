module('walnut.pcfg.startify', package.seeall)
...
local version_header_line = '[ walnut, last updated: ' .. require('walnut.version').last_updated_time .. ' ]'
local maintainer_header_line = '< MAINTAINER: Hawtian Wang(twistoy.wang@gmail.com) >'
vim.list_extend(header, {''})
vim.list_extend(header, {
  string.rep(' ', max_length - #version_header_line) .. version_header_line
})
vim.list_extend(header, {
  string.rep(' ', max_length - #maintainer_header_line) .. maintainer_header_line
})
