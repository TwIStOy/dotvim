...
local function is_uncountable(win_id)
  ...
  return vim.tbl_contains(uncountable_type, ft) or
             vim.tbl_contains(uncountable_type, bt)
end
...
local function is_not_blocking_vim_quit(win_id)
  ...
  return vim.tbl_contains(auto_close_type, ft) or
             vim.tbl_contains(auto_close_type, bt)
end
...
local function skip_uncountable_window(cnt)
  ...
  for k, win_id in pairs(va.nvim_list_wins()) do
    if not is_uncountable(win_id) then
      rest = rest - 1
      if rest == 0 then return win_id end
    end
  end
...
function goto_win(id)
  ...
  if win_id > 0 then va.nvim_set_current_win(win_id) end
end
...
function fast_forward_to_file_explorer()
  ...
  if packer_plugins['nvim-tree.lua'] and
      not packer_plugins['nvim-tree.lua'].loaded then
...
function check_last_window()
  ...
  for i = 1, n do
    local win_id = vfun('win_getid', {i})
    if is_not_blocking_vim_quit(win_id) then total = total - 1 end
  end
