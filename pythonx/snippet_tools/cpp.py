...
_POSTFIX_FOR_I_LOOP_VALUE = """for (auto i = 0u; i < ranges::size(`!p snip.rv = match.group(1)`); i++) {
  ${0:/* body */}
}"""

_POSTFIX_FOR_R_LOOP_VALUE = """for (auto i = ranges::ssize(`!p snip.rv = match.group(1)`); i >= 0; i--) {
  ${0:/* body */}
}"""
...
def register_postfix_snippets():
  ...
  postfix_mappings = {
    "for":  [ _POSTFIX_FOR_LOOP_VALUE, "range-base for" ],
    'fori': [ _POSTFIX_FOR_I_LOOP_VALUE, "for loop"],
    'forr': [ _POSTFIX_FOR_R_LOOP_VALUE, "reversed for loop"],
    'if':   [ _POSTFIX_IF_VALUE, "if-expr"],
    'be':   [ _POSTFIX_BEGIN_END_VALUE, "begin-end"],
    'mv':   [ _POSTFIX_STD_MOVE_VALUE, "std::move($expr)"],
    'fwd':  [ _POSTFIX_STD_FORWARD_VALUE, "std::forward($expr)"],
